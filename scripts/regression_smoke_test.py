...
def run_smoke_test():
	...
	try:
		...
		print("  Created: " + bill.name + " | bill_number: " + str(bill.bill_number) +
		      " | bill_entries: " + str(len(bill.bill_entries)))
		assert bill.bill_number == 1
		# Variation build 2: bill_entries is the editable surface (one row
		# per item). items is GENERATED by the allocator after save with
		# non-zero cumulative_qty.
		assert len(bill.bill_entries) == 3, "Expected 3 bill_entries, got " + str(len(bill.bill_entries))

		# Enter cumulative quantities on the bill_entries surface
		for be in bill.bill_entries:
			if be.item_no == "1.1":
				be.cumulative_qty = 30   # 30 cum @ 5000 = 150000
			elif be.item_no == "1.2":
				be.cumulative_qty = 50   # 50 sqm @ 500 = 25000
			elif be.item_no == "2.1":
				be.cumulative_qty = 20   # 20 m @ 1500 = 30000
		bill.save()
		bill.reload()
		# After save, items is generated — one per scope per item with
		# this_bill_qty > 0. WO has no variations, so 1 scope per item.
		assert len(bill.items) == 3, "Expected 3 generated items, got " + str(len(bill.items))
		for r in bill.items:
			assert r.boq_row_uid and len(r.boq_row_uid) == 36, "RA Bill item missing boq_row_uid: " + str(r.item_no)
			assert r.scope_source == "Original", "Expected Original scope, got " + str(r.scope_source)
		...
		print("  All RA Bill items have boq_row_uid + valid summary_head + Original scope")
		...
		# Prior cumulative on item 1.1 = 30 (from bill1). Push to 101 — over the 100 ceiling.
		for be in bill2.bill_entries:
			if be.item_no == "1.1":
				be.cumulative_qty = 101
			elif be.item_no == "1.2":
				be.cumulative_qty = 60
			elif be.item_no == "2.1":
				be.cumulative_qty = 30
		bill2.save()
		caught = False
		try:
			bill2.submit()
		...
		for be in bill3.bill_entries:
			if be.item_no == "1.1":
				be.cumulative_qty = 30
			elif be.item_no == "1.2":
				be.cumulative_qty = 212   # 6% over BOQ qty 200
			elif be.item_no == "2.1":
				be.cumulative_qty = 20
		bill3.save()
		caught = False
		try:
			bill3.submit()
