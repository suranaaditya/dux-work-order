...
class CivilWorkOrderBOQ(Document):
	...
	def validate_wo_is_submitted(self):
		if not self.civil_work_order:
			return
		wo_docstatus = frappe.db.get_value("Work Order Contract", self.civil_work_order, "docstatus")
		if wo_docstatus != 1:
			frappe.throw(_(
				"Linked Work Order Contract {0} must be Submitted before a BOQ can be saved against it. "
				"Current status: {1}"
			).format(self.civil_work_order,
					 {0: "Draft", 2: "Cancelled"}.get(wo_docstatus, wo_docstatus)))

	def set_default_deviation_limits(self):
		# Pull default from Work Order Settings if a line has no explicit limit
		default_limit = None
		try:
