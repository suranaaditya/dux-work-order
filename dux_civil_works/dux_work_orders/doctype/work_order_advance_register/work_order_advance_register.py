...
class WorkOrderAdvanceRegister(Document):
	def validate(self):
	...
	def validate_wo_is_submitted(self):
		...
		if wo_docstatus != 1:
			frappe.throw(_(
				"Linked Work Order Contract {0} must be Submitted before an "
...
def get_or_create_register(civil_work_order):
	...
	existing = frappe.db.exists(
		"Work Order Advance Register", {"civil_work_order": civil_work_order}
	)
	if existing:
		return frappe.get_doc("Work Order Advance Register", existing)

	wo = frappe.get_doc("Work Order Contract", civil_work_order)
...
def get_outstanding_balance(civil_work_order, advance_type):
	...
	if not fieldname:
		return 0
	return float(frappe.db.get_value("Work Order Advance Register", reg_name, fieldname) or 0)
